...
import { FaBell, FaUserFriends, FaEdit, FaUserEdit } from "react-icons/fa";
            ...
            <div className="feed_layout">
                            ...
                            <div className="miniProfile_top">
                                ...
                                <div className="miniProfile_user-Info">
                                    ...
                                    <div className="miniProfile_title">
                                        {/* {user.role === 'ADMIN' ? 'מנהל.ת מערכת' : 'משתמש.ת רגיל.ה'} */}
                                        {user ? "משתמש.ת מחובר.ת" : 'אורח'}
                                    </div>
                                </div>
                            </div>
                            ...
                            {arrTopUsers?.length > 0 && (
                                <div className="miniProfile_usersRow">
                                    {arrTopUsers.map((user, i) => (
                                        <span key={i} className="topUser"><span className="n">{getMedal(i)}</span>{user.userName.length > 7 ? user.userName.slice(0, 7) + "..." : user.userName}</span>
                                    ))}
                                </div>
                            )}
                        </div>
                ...
                <div className="feed_part" >
                    ...
                    <div className="mobile-network-filter">
                        <img src={networkIcon} className="networkIcon" onClick={() => setShowMobileAddFriend(!showMobileAddFriend)} />
                        <img src={filterIcon} className="filterIcon" onClick={() => setShowMobileFilter(!showMobileFilter)} />
                    </div>
                                                        ...
                                                        <div onClick={() => fetchToProfile(comment.userId?._id)}>
                                                            {comment.userId?.profilePicture ? (
                                                                ...
                                                                <div className="avatar-fallback" id="avatar-fallback_inAddComment">
                                                                    {(comment.userId?.userName || 'אורח').charAt(0).toUpperCase()}
                                                                </div>
                                                            )}
                                                        </div>
                                                    </div>
